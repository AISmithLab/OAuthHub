import type { NextApiRequest } from "next";
import { db } from "~/server/db";
import { type OAuthHubVersionId } from "~/lib/demo-config";
import { oauthHubAuthPath } from "~/server/storage";

type OAuthHubVerifier = {
  verifySignedPayload: <T>(opts: {
    headers: NextApiRequest["headers"];
    rawBody: string;
    publicKeyJwk: unknown;
    requireSignature?: boolean;
  }) => Promise<{ body: T; signature: string | null }>;
};

// eslint-disable-next-line @typescript-eslint/no-var-requires
const OAuthHubClient = require("~/lib/oauthub-client") as OAuthHubVerifier;

export const oauthHubApiConfig = { api: { bodyParser: false } };

export class OAuthHubRequestError extends Error {
  statusCode: number;

  constructor(statusCode: number, message: string) {
    super(message);
    this.name = "OAuthHubRequestError";
    this.statusCode = statusCode;
  }
}

function readRawBody(req: NextApiRequest): Promise<string> {
  return new Promise((resolve, reject) => {
    let data = "";
    req.on("data", (chunk: Buffer) => { data += chunk.toString(); });
    req.on("end", () => resolve(data));
    req.on("error", reject);
  });
}

async function getStoredPublicKey(version: OAuthHubVersionId): Promise<unknown> {
  try {
    return (await db.getData(oauthHubAuthPath(version)) as { publicKey?: unknown }).publicKey;
  } catch {
    throw new OAuthHubRequestError(401, "No OAuthHub session found");
  }
}

export async function verifyOAuthHubRequest<T>(
  req: NextApiRequest,
  version: OAuthHubVersionId
): Promise<{ body: T; rawBody: string }> {
  const rawBody = await readRawBody(req);
  const publicKeyJwk = await getStoredPublicKey(version);
  if (!publicKeyJwk) {
    throw new OAuthHubRequestError(401, "Missing OAuthHub public key");
  }

  try {
    const result = await OAuthHubClient.verifySignedPayload<T>({
      headers: req.headers,
      rawBody,
      publicKeyJwk,
    });
    return { body: result.body, rawBody };
  } catch (error) {
    if (error instanceof Error) {
      if (error.message === "Missing OAuthHub signature") {
        throw new OAuthHubRequestError(401, error.message);
      }
      if (error.message === "Invalid signature") {
        throw new OAuthHubRequestError(401, error.message);
      }
      if (error.message === "Invalid JSON payload") {
        throw new OAuthHubRequestError(400, error.message);
      }
    }
    throw error;
  }
}

export function isOAuthHubRequestError(error: unknown): error is OAuthHubRequestError {
  return error instanceof OAuthHubRequestError;
}
