...
const DANGEROUS_REGEX_PATTERNS = /(\{[0-9]{4,}\})|(\(\?[^)]*\)\*)|(\(.*\+\)\+)|(\(\.\*\)\{)|(\[\^?\]?\]?\*\{)/;
...
function createSafeRegex(pattern, flags) {
  ...
  if (DANGEROUS_REGEX_PATTERNS.test(pattern)) {
    throw new Error('Regex pattern contains potentially dangerous constructs (possible ReDoS)');
  }
  ...
  if (value === 'NOW') {
    ...
    return value.slice(1, -1).replace(/\\(.)/g, (_, ch) => {
      switch (ch) {
        ...
        case '\\': return '\\';
        case '"': return '"';
        default: return '\\' + ch;  // preserve unknown escapes (e.g. \b, \s for regex)
      }
    });
  } else if (value.startsWith('[') && value.endsWith(']')) {
...
class Runtime {
  ...
  _getResourceConstraints() {
    return this.constraints?.resource || {};
  }

  _normalizeConstraintValues(values) {
    return Array.isArray(values)
      ? values.map(value => String(value).trim().toLowerCase()).filter(Boolean)
      : [];
  }

  _mimeTypeAllowed(mimeType, allowedFileTypes) {
  ...
  _resourceItemAllowed(resourceType, item) {
    const resource = this._getResourceConstraints();
    if (!item || typeof item !== 'object') {
      return true;
    }

    if (resourceType === 'gmail') {
      const allowedLabels = this._normalizeConstraintValues(resource.allowedLabels);
      if (allowedLabels.length > 0) {
        const itemLabels = this._normalizeConstraintValues(item.labelIds);
        return itemLabels.some(label => allowedLabels.includes(label));
      }
    }

    if (resourceType === 'google_drive') {
      const allowedFileTypes = this._normalizeConstraintValues(resource.allowedFileTypes);
      if (allowedFileTypes.length > 0 && !this._mimeTypeAllowed(item.mimeType, allowedFileTypes)) {
        return false;
      }

      const allowedFolders = this._normalizeConstraintValues(resource.allowedFolders);
      if (allowedFolders.length > 0) {
        const parentNames = this._normalizeConstraintValues(item.parents);
        return parentNames.some(parentName => allowedFolders.includes(parentName));
      }
    }
  ...
  _applyPullConstraints(resourceType, result) {
    const dataKey = Object.keys(result || {})[0];
    if (!dataKey || !Array.isArray(result[dataKey])) {
      return result;
    }

    const filteredItems = result[dataKey].filter(item => this._resourceItemAllowed(resourceType, item));
    return { ...result, [dataKey]: filteredItems };
  }

  _obfuscatePath(target, pathSegments) {
    if (target == null || pathSegments.length === 0) {
      return;
    }

    if (Array.isArray(target)) {
      for (const entry of target) {
        this._obfuscatePath(entry, pathSegments);
      }
      return;
    }
    ...
    if (rest.length === 0) {
      target[segment] = '[OBFUSCATED]';
      return;
    }

    this._obfuscatePath(target[segment], rest);
  }

  applyOutputConstraints(data) {
    const resource = this._getResourceConstraints();
    const fieldsToObfuscate = Array.isArray(resource.obfuscateFields)
      ? resource.obfuscateFields.map(field => String(field).trim()).filter(Boolean)
      : [];

    if (fieldsToObfuscate.length === 0 || data == null) {
      return data;
    }

    const cloned = JSON.parse(JSON.stringify(data));
    for (const fieldPath of fieldsToObfuscate) {
      this._obfuscatePath(cloned, fieldPath.split('.').filter(Boolean));
    }
    return cloned;
  }

  _validateWriteConstraints(resourceType, payload) {
    if (resourceType !== 'google_drive') {
      return;
    }
    ...
    const resource = this._getResourceConstraints();
    const allowedFileTypes = this._normalizeConstraintValues(resource.allowedFileTypes);
    if (allowedFileTypes.length > 0 && !this._mimeTypeAllowed(payload.mimeType, allowedFileTypes)) {
      throw new Error('Write blocked by constraints: file type is not allowed.');
    }

    const allowedFolders = this._normalizeConstraintValues(resource.allowedFolders);
    if (allowedFolders.length > 0) {
      const parentNames = this._normalizeConstraintValues(payload.parents);
      const parentAllowed = parentNames.some(parentName => allowedFolders.includes(parentName));
      if (!parentAllowed) {
        throw new Error('Write blocked by constraints: destination folder is not allowed.');
      }
    }
  }
  ...
  _matchesFilterOperation(value, operation, target) {
    ...
    switch (operation) {
      ...
      case 'match': {
        try {
          const regex = createSafeRegex(target);
          return values.some(entry => regex.test(String(entry ?? '')));
        } catch (e) {
          console.error(`Invalid or unsafe regex pattern in filter match: ${e.message}`);
          return false;
        }
      }
      default:
  ...
  filter(data, name, operation, field, targetValue = null, pattern = null, requirement = null) {
    ...
    const target = pattern || targetValue;
    ...
    for (const item of data) {
      for (const f of field) {
        ...
        for (const regex of pattern) {
          try {
            // Handle regex patterns properly - patterns should come as raw strings
            // If the pattern is already escaped for JSON, don't double-escape
            let regexPattern = regex;
            
            // Create RegExp object safely with length and complexity limits
            const regexObj = createSafeRegex(regexPattern, 'g');
  ...
  async post(name, destination, data) {
    ...
    try {
      ...
      console.log(`Operator ${name}: privateKey present:`, !!this.privateKey, 'type:', this.privateKey?.constructor?.name);
      if (this.privateKey) {
        try {
          ...
          console.log(`Operator ${name}: Signature created successfully`);
        } catch (signErr) {
          console.error(`Operator ${name}: SIGNING FAILED:`, signErr.message, signErr.stack);
          throw new Error(`Payload signing failed: ${signErr.message}`);
        }
      } else {
        console.error(`Operator ${name}: NO PRIVATE KEY — signature will be missing`);
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(`HTTP error! status: ${response.status} - ${errorBody} [key=${!!this.privateKey}, sig=${!!headers['X-OAuthHub-Signature']}]`);
      }
  ...
  async _createDriveFile(token, payload) {
    const boundary = `oauthhub_${crypto.randomUUID()}`;
    const metadata = {
      name: payload.name,
  ...
  async write(name, resourceType, action, data, manifest) {
    ...
    this._validateWriteConstraints(resourceType, payload);
  ...
  executeManifestPreview(manifestText) {
    ...
    for (const operatorName of manifest.pipeline) {
      ...
      switch (type) {
        ...
        case 'debug':
          // Debug is a passthrough — data flows through unchanged
          break;
  ...
  async executeManifest(manifestText, opts = {}) {
    ...
    console.log(`🚀 RUNTIME: Executing ${manifest.title}: ${manifest.description}`);
    console.log(`🛤️ Pipeline: ${manifest.pipeline.join(' -> ')}`);
    ...
    for (const operatorName of manifest.pipeline) {
      ...
      console.log(`🔄 Executing operator: ${operatorName} (${config.type})`);
      console.log(`📝 Config:`, JSON.stringify(config, null, 2));
      console.log(`📊 Input data:`, currentData ? JSON.stringify(currentData).substring(0, 200) + '...' : 'null');

      const operatorType = config.type.toLowerCase();

      if (operation === 'read' && currentData == null && !['pull', 'input'].includes(operatorType)) {
        console.log(`⏭️ Skipping operator during read before data exists: ${operatorName}`);
        continue;
      }
      ...
      switch (operatorType) {
        ...
        case 'post':
          currentData = this.applyOutputConstraints(currentData);
        ...
        case 'write':
          ...
          if (operation === 'write') {
            return this.applyOutputConstraints(currentData);
          }
          break;

        case 'debug':
          // Debug is a passthrough — data flows through unchanged
          console.log(`🐛 DEBUG [${operatorName}]:`, JSON.stringify(currentData).substring(0, 500));
          break;
      ...
      console.log(`${operatorName} output:`, currentData);
    }
  ...
  /**
   * Execute manifest in debug mode — captures data snapshots at Debug operators.
   * Uses sample data (or mockData if provided) so it doesn't require real Google auth.
   * @param {string} manifestText
   * @param {Array|Object|null} mockData - optional data injected by IDE Mock panel
   */
  executeManifestDebug(manifestText, mockData = null) {
    const manifest = parseManifest(manifestText);
    const debugSnapshots = [];

    // Check if there's a Mock operator in the pipeline
    const hasMockOperator = manifest.pipeline.some(name => {
      const op = manifest.operators[name];
      return op && String(op.type).toLowerCase() === 'mock';
    });

    // Determine initial data
    let currentData;
    if (mockData !== null && hasMockOperator) {
      // Use mock data from the IDE panel
      if (Array.isArray(mockData)) {
        currentData = { data: JSON.parse(JSON.stringify(mockData)) };
      } else if (typeof mockData === 'object') {
        currentData = JSON.parse(JSON.stringify(mockData));
      } else {
        currentData = { data: mockData };
      }
    } else {
      // Find the Pull/Input operator to determine resource type
      let resourceType = 'unknown';
      ...
      const sample = Runtime._sampleData(resourceType);
      currentData = { [sample.dataKey]: JSON.parse(JSON.stringify(sample.data)) };
    }
    ...
    for (const operatorName of manifest.pipeline) {
      ...
      switch (type) {
        ...
        case 'mock':
          // Mock operator: data already seeded above; just pass through
          break;
        ...
        case 'debug':
          // Capture snapshot — data passes through unchanged
          debugSnapshots.push({
    ...
    return { debugSnapshots, manifest };
  }
}
